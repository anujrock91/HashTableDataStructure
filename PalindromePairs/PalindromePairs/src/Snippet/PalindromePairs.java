...
public class PalindromePairs {
	...
	public List<List<Integer>> getPalindromePairs(String [] words){
		...
		for(int i=0;i<words.length;i++){
			...
			if(wordIndex.containsKey(reverse) && !reverse.equalsIgnoreCase(s)){
			...
			for(int c=1;c<s.length();c++){ //ignoring if the whole string is a palindrome because it is already considered above
			...
			for(int c=s.length()-1;c>=0;c--){ //ignoring if the whole string is a palindrome because it is already considered above
	...
	public static void main(String args[]){
		...
		result = new PalindromePairs().getPalindromePairs(new String[]{"ab","a"});
