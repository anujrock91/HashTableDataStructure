...
public class UniqueWordAbbrevation {
	...
	public UniqueWordAbbrevation(String [] list){
		...
		for(String s: words){
			...
			if(wordUnique.containsKey(abbrevation)){
				wordUnique.put(abbrevation, false);
			}
			else{
				wordUnique.put(abbrevation, true);
			}
		}
	}
	...
	public static void main(String args[]){
		UniqueWordAbbrevation obj = new UniqueWordAbbrevation(new String[]{"deer", "door", "cake", "card","a","a"});
		System.out.println(
				obj.isUnique("dear") + " " + obj.isUnique("cart") + " "+ 
				obj.isUnique("cane")+" " + obj.isUnique("make"));
